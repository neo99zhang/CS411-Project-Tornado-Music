...
def like(flag:int,uid:int)->None:
    ...
    query='INSERT INTO LIKE_LIST VALUES ('+ str(user_id)+','+str(flag)+','+'1);'
    print(query)
    # if(conn.execute(query_if_exist).fetchall()==[]):
    #     query='INSERT INTO LIKE_LIST VALUES ('+ str(user_id)+','+str(flag)+','+'1);'
    # print(query)
...
def fetch_todo(uid:int) -> dict:
    ...
    for result in query_results:
        ...
        query="SELECT How_much FROM LIKE_LIST WHERE User_id="+str(uid) +" AND Music_id="+str(result[0])+";"
        ...
        lis=[i[0] for i in query_results2 ];
